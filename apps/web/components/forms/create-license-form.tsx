...
import { Key, Layers, Clock, AlertCircle, CheckCircle2, Info, StickyNote, Loader2, RefreshCw, Lock, Users } from "lucide-react";
...
export function CreateLicenseForm({ onClose, onSuccess }: CreateLicenseFormProps) {
  const [formData, setFormData] = useState({
    ...
    expiry_days: 30, note: "", hwid_lock: false, user_id: "",
  });
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (formData.use_custom_key && formData.custom_key) body.key = formData.custom_key;
      ...
      {generatedKey ? (
        <div className="text-center py-4">
          ...
          <div className="rounded-xl border border-emerald-500/20 bg-emerald-500/5 p-4">
            ...
            <div className="text-lg font-mono font-bold text-white break-all">{generatedKey}</div>
          </div>
        </div>
      ) : (
        <>
          ...
          {/* Link User (Optional) - For License Login */}
          <div>
            <label className="block text-xs font-semibold text-zinc-400 uppercase tracking-wider mb-1.5">
              Link User ID <span className="text-zinc-600">(Optional - for license login)</span>
            </label>
            <div className="relative">
              ...
              <input 
                ...
                className="w-full rounded-xl border border-zinc-700/50 bg-zinc-800/50 pl-10 pr-4 py-2.5 text-sm text-white placeholder-zinc-500 focus:border-indigo-500/50 focus:ring-2 focus:ring-indigo-500/20 focus:outline-none" 
              />
            </div>
            <p className="text-xs text-zinc-500 mt-1"><Info className="h-3 w-3 inline" /> Link user for license-based login</p>
          </div>
