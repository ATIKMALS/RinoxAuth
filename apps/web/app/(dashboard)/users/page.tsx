...
export default async function UsersPage() {
  ...
  const userAppIds = apps.map((a: any) => a.id);
  const allUsers = await getUsers();
  
  // ✅ FIX: Hide auto-generated users (username starting with "user_")
  const filteredUsers = allUsers.filter((u: any) => !u.username.startsWith("user_"));
  const users = filteredUsers.filter((u: any) => userAppIds.includes(u.application_id));
